...
exports['qb-target']:AddTargetBone(headlights, {
    options = {
        {
            event = "case-rgbcontroller:client:InstallXenons",
            icon = "fas fa-bolt",
...
exports['qb-target']:AddTargetBone(doors, {
    options = {
        {
            event = "case-rgbcontroller:client:InstallNeons",
            icon = "fas fa-bolt",
...
RegisterNetEvent('case-rgbcontroller:client:InstallXenons', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.5) then
		...
		if DoesEntityExist(vehicle) then
			...
			QBCore.Functions.Progressbar("install_xenon", Config.Lang['xenonprogbar'], Config.XenonInstallTime, false, true, {
			...
			TriggerServerEvent('case-rgbcontroller:server:InstallXenons')
		end)
		end	
	else
...
RegisterNetEvent('case-rgbcontroller:client:InstallNeons', function()
	local playerPed	= PlayerPedId()
    local coords = GetEntityCoords(playerPed)
    if IsPedSittingInAnyVehicle(playerPed) then
		QBCore.Functions.Notify(Config.Lang['invehicle'], "error", 3500)
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.5) then
		...
		if DoesEntityExist(vehicle) then
			...
			QBCore.Functions.Progressbar("install_neons", Config.Lang['neonkitprogbar'], Config.NeonInstallTime, false, true, {
			disableMovement = true,
			disableCarMovement = false,
			disableMouse = false,
			disableCombat = true,
			...
			CurrentVehicleData = QBCore.Functions.GetVehicleProperties(vehicle)
			TriggerServerEvent('updateVehicle', CurrentVehicleData)
			TriggerServerEvent('case-rgbcontroler:server:InstallNeons')
		end)
		end	
	else
		QBCore.Functions.Notify(Config.Lang['outrange'], "error", 3500)
